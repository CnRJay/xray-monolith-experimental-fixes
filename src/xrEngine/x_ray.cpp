...
LPCSTR xr_ToUTF8(LPCSTR input, int max_length) {
  ...
  int32_t conv_len = ucnv_toUChars(conv_from, &converted[0], converted.size(),
                                   input, strlen(input), &errorCode);
  ...
  int32_t u8_len = ucnv_fromUChars(conv_u8, &g[0], g.size(), &converted[0],
    ...
    // Health
      ...
      if (local_info.ironman && local_info.lives_left) {
        ...
        if (real_lives == 0 || real_lives > 1)
          ...
          snprintf(state_buffer, 128, "%s | %i %s", discord_strings.dead,
                   real_lives, discord_strings.livesleftsingle);
      }
...
void Init_Discord() {
  auto result = discord::Core::Create(
      477910171964801060, DiscordCreateFlags_NoRequireDiscord, &discord_core);
...
void Startup()
{
  ...
  if (use_reshade)
    ...
    Msg("[ReShade]: ReShade not installed or version too old - didn't load "
        "compatibility addon");
...
extern BOOL DllMainXrCore(HANDLE hinstDLL, DWORD ul_reason_for_call,
                          LPVOID lpvReserved);
extern BOOL DllMainXrPhysics(HANDLE hModule, DWORD ul_reason_for_call,
                             LPVOID lpReserved);
