...
#ifndef _EDITOR
  Log("IGame_Persistent::OnAppStart start");
  Environment().load();
  Log("IGame_Persistent::OnAppStart end");
#endif
...
void IGame_Persistent::Prefetch() {
  ...
  if (m_textures_prefetch_config->section_exist("prefetch_folders")) {
    ...
    for (CInifile::SectCIt I = sect_f.Data.begin(); I != sect_f.Data.end();
         I++) {
...
void IGame_Persistent::destroy_particles(const bool &all_particles) {
  ...
  if (all_particles) {
    ...
    CPS_Instance **I =
        (CPS_Instance **)_alloca(active_size * sizeof(CPS_Instance *));
    ...
    CPS_Instance **E =
        std::remove_if(I, I + active_size, &destroy_on_game_load::predicate);
    for (; I != E; ++I)
...
void IGame_Persistent::GrassBendersUpdate(u16 id, u8 &data_idx, u32 &data_frame,
                                          Fvector &position, float init_radius,
                                          float init_str, bool CheckDistance) {
  ...
  float rad =
      data_idx == NULL
          ? 1.0
          : std::max(1.0f, grass_shader_data.radius_curr[data_idx] + 0.5f);
  ...
  if (data_idx == NULL) {
    ...
    if (grass_shader_data.id[idx] == NULL) {
      data_idx = idx;
      GrassBendersSet(idx, id, position, Fvector3().set(0, -99, 0), 0, 0, 0.0f,
                      init_radius, BENDER_ANIM_DEFAULT, true);
...
void IGame_Persistent::GrassBendersAddExplosion(u16 id, Fvector position,
                                                Fvector3 dir, float fade,
  ...
  for (int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++) {
    ...
    if (grass_shader_data.anim[idx] != BENDER_ANIM_EXPLOSION) {
      ...
      GrassBendersSet(idx, id + 99, position, dir, fade, speed, intensity,
                      radius, BENDER_ANIM_EXPLOSION, true);
...
void IGame_Persistent::GrassBendersAddShot(u16 id, Fvector position,
                                           Fvector3 dir, float fade,
                                           float speed, float intensity,
                                           float radius) {
  // Is disabled?
  ...
  for (int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++) {
    ...
    if (grass_shader_data.id[idx] == id) {
      ...
      GrassBendersSet(idx, id, position, dir, fade, speed, currentSTR, radius,
                      BENDER_ANIM_EXPLOSION, false);
      grass_shader_data.str_target[idx] += intensity;
      AddAt = -1;
      break;
    } else {
  ...
  for (int idx = 1; idx < ps_ssfx_grass_interactive.y + 1; idx++) {
    if (grass_shader_data.id[idx] != NULL) {
                                  ...
                                  // BENDER_ANIM_PULSE for anomalies )
      {
        ...
        // Easing
        float diff =
            abs(grass_shader_data.str[idx] - grass_shader_data.str_target[idx]);
        diff = std::max(0.1f, diff);

        // Intensity
        if (grass_shader_data.str_target[idx] <= grass_shader_data.str[idx]) {
          grass_shader_data.str[idx] -=
              Device.fTimeDelta * grass_shader_data.fade[idx] * diff;
        } else {
          grass_shader_data.str[idx] +=
              Device.fTimeDelta * grass_shader_data.speed[idx] * diff;
      ...
      case BENDER_ANIM_WAVY: {
        // Anim Speed
        grass_shader_data.time[idx] +=
            Device.fTimeDelta * 1.5f * grass_shader_data.speed[idx];

        // Curve
        float curve = sin(grass_shader_data.time[idx]);

        // Intensity using curve
        grass_shader_data.str[idx] = curve * cos(curve * 1.4f) * 1.8f *
                                     grass_shader_data.str_target[idx];
      }
      ...
      case BENDER_ANIM_SUCK: {
        ...
        // Perlin Noise
        float curve =
            clampr(PerlinNoise1D->GetContinious(grass_shader_data.time[idx]) +
                       0.5f,
                   0.f, 1.f) *
            -1.0;
      ...
      case BENDER_ANIM_BLOW: {
        // Anim Speed
        grass_shader_data.time[idx] +=
            Device.fTimeDelta * 1.2f * grass_shader_data.speed[idx];
      ...
      case BENDER_ANIM_PULSE: {
        ...
        if (grass_shader_data.radius_curr[idx] >= grass_shader_data.radius[idx])
          grass_shader_data.str[idx] +=
              GrassBenderToValue(grass_shader_data.str[idx], 0.0f,
      ...
      case BENDER_ANIM_DEFAULT:
        ...
        grass_shader_data.str[idx] +=
            GrassBenderToValue(grass_shader_data.str[idx],
...
void IGame_Persistent::GrassBendersReset(u8 idx) {
  // Reset Everything
  GrassBendersSet(idx, NULL, Fvector3().set(0, 0, 0), Fvector3().set(0, -99, 0),
                  0, 0, 0, 0, BENDER_ANIM_DEFAULT, true);
  grass_shader_data.str_target[idx] = 0;
}

void IGame_Persistent::GrassBendersSet(u8 idx, u16 id, Fvector position,
                                       Fvector3 dir, float fade, float speed,
                                       float intensity, float radius,
                                       GrassBenders_Anim anim, bool resetTime) {
  // Set values
...
float IGame_Persistent::GrassBenderToValue(float &current, float go_to,
                                           float intensity, bool use_easing) {
