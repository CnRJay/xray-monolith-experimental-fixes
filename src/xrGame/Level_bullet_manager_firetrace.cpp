        ...
        if (Fsphere::rpNone !=
          ...
          if (actor) {
            ...
            if (Random.randF(0.f, 1.f) > (ahp * hpf)) {
              ...
              if (cform->_RayQuery(rd, temp_rq_results)) {
                bRes = TRUE;        // hit actor
...
void CBulletManager::DynamicObjectHit(CBulletManager::_event &E) {
  ...
  if (E.bullet.flags.allow_sendhit && !E.Repeated) {
    ...
    Hit.GenHeader(u16((AddStatistic) ? GE_HIT_STATISTIC : GE_HIT) & 0xffff,
                  E.R.O->ID());
...
bool CBulletManager::ObjectHit(SBullet_Hit *hit_res, SBullet *bullet,
  ...
  if (!g_mt_config.test(mtBullets) &&
      ai().script_engine().functor("_g.CBulletManager__ObjectHit", funct))
    funct(bullet->section,
          smart_cast<CGameObject *>(R.O)
              ? smart_cast<CGameObject *>(R.O)->lua_game_object()
              : nullptr,
          end_point, bullet->dir,
          *GMLib.GetMaterialByIdx(target_material)->m_Name, bullet->speed,
          bullet->weapon_id);
