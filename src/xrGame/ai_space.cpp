...
CAI_Space::CAI_Space() {
  m_ef_storage = 0;
  m_game_graph = 0;
  m_graph_engine_max_vertex_count = 0;
  m_cover_manager = 0;
  m_level_graph = 0;
  m_alife_simulator = 0;
...
void CAI_Space::init() {
  ...
  m_graph_engine_max_vertex_count = 1024;
...
CAI_Space::~CAI_Space() {
  ...
  for (CGraphEngine *&engine : m_graph_engines) {
    xr_delete(engine);
  }
  m_graph_engines.clear();
...
void CAI_Space::load(LPCSTR level_name) {
  ...
  m_graph_engine_max_vertex_count = _max(game_graph().header().vertex_count(),
...
void CAI_Space::unload(bool reload) {
  ...
  if (!reload && m_game_graph)
    m_graph_engine_max_vertex_count = game_graph().header().vertex_count();
}
...
void CAI_Space::game_graph(CGameGraph *game_graph) {
  ...
  m_graph_engines.clear();
  m_graph_engine_max_vertex_count = this->game_graph().header().vertex_count();
}
...
void CAI_Space::create_graph_engine(CGraphEngine *&engine) const {
  engine = xr_new<CGraphEngine>(m_graph_engine_max_vertex_count);
}
