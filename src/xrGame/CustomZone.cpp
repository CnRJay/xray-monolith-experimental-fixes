...
#define FASTMODE_DISTANCE                                                      \
  (100.f) // distance to camera from sphere, when zone switches to fast update
          // sequence
...
void CCustomZone::Load(LPCSTR section) {
  ...
  if (pSettings->line_exist(section, "blowout_disable_idle")) {
    m_zone_flags.set(eBlowoutDisableIdle,
  ...
  if (m_zone_flags.test(eIdleLight)) {
    ...
    volumetric_distance = READ_IF_EXISTS(pSettings, r_float, section,
                                         "idle_light_volumetric_distance", .5f);
    volumetric_intensity = READ_IF_EXISTS(
        pSettings, r_float, section, "idle_light_volumetric_intensity", .5f);
    volumetric_quality = READ_IF_EXISTS(pSettings, r_float, section,
    ...
    for (OBJECT_INFO_VEC_IT it = m_ObjectInfoMap.begin();
      ...
      if (pEntityAlive) {
        ...
        if (ai().script_engine().functor(
          ...
          info.zone_ignore =
              !funct(this->lua_game_object(), pObject->lua_game_object());
      }
        ...
        if (GData.id[grassbender_id] == ID()) {
          GData.str_target[grassbender_id] +=
              g_pGamePersistent->GrassBenderToValue(
                  GData.str_target[grassbender_id], 0.0f, 4.0f, false);

          // Remove ( Don't worry, GrassBenderToValue() it's going to get the ==
          // 0 )
...
void CCustomZone::GrassZoneUpdate() {
  ...
  if (targetAnim > 0 || (grassbender_id > 0 && GData.anim[grassbender_id] > 0))
    g_pGamePersistent->GrassBendersUpdate(ID(), grassbender_id,
                                          grassbender_frame, Position(),
  ...
  if (GData.anim[grassbender_id] != targetAnim) {
    GData.str_target[grassbender_id] += g_pGamePersistent->GrassBenderToValue(
  ...
  if (IsActive) {
    if (m_BendGrass_whenactive_speed >= 0)
      GData.speed[grassbender_id] += g_pGamePersistent->GrassBenderToValue(
          GData.speed[grassbender_id], m_BendGrass_whenactive_speed, 10.0f,
          true);

    if (m_BendGrass_whenactive_str >= 0)
      GData.str_target[grassbender_id] += g_pGamePersistent->GrassBenderToValue(
          GData.str_target[grassbender_id], m_BendGrass_whenactive_str, 10.0f,
          true);
  } else {
    GData.speed[grassbender_id] += g_pGamePersistent->GrassBenderToValue(
        GData.speed[grassbender_id], m_BendGrass_idle_speed, 10.0f, true);
    GData.str_target[grassbender_id] += g_pGamePersistent->GrassBenderToValue(
        GData.str_target[grassbender_id], m_BendGrass_idle_str, 10.0f, true);
  }
}
