...
struct LuaLock {
  CRITICAL_SECTION cs;
  LuaLock() { InitializeCriticalSection(&cs); }
  ~LuaLock() { DeleteCriticalSection(&cs); }
  void Enter() { EnterCriticalSection(&cs); }
  void Leave() { LeaveCriticalSection(&cs); }
};
static LuaLock lua_lock;
      ...
      g_allow_silencer_hide_tracer) // momopate: Make it optional
  {
    ...
    if (g_obj) {
      ...
      if (weapon && weapon->IsSilencerAttached())
        flags.allow_tracer =
            (weapon->GetSilencedTracers()); // momopate: customize per weapon
    }
  }
...
void populateBulletTable(::luabind::object &table, const Fvector &position,
                         const Fvector &direction, float speed, float fly_dist,
                         LPCSTR catridgeSection, u32 bulletId, u16 weaponId,
                         u16 parentId, u16 targetId, LPCSTR materialName,
                         float life_time, int element) {
...
void CBulletManager::Render() {
  ...
  // 0-riochet
  // 1-za_stryavanie_puli_v_materiale
  // 2-probivanie_materiala
  if (g_bDrawBulletHit) {
  ...
  for (BulletVecIt it = m_BulletsRendered.begin();
    ...
    if (length < m_fTracerLengthMin) {
      // momopate: Optionally cap tracers to their minimum length instead of
      // not rendering them
      ...
      if (ai().script_engine().functor("_G.CBulletOnImpact", funct)) {
        ...
        populateBulletTable(
            ...
            !fis_zero(end_point.x) && !fis_zero(end_point.y) &&
                ...
                ? end_point
                : bullet->bullet_pos,
            bullet->dir, bullet->speed, bullet->fly_dist + E.R.range,
            bullet->catridgeSection, bullet->bulletId, bullet->weapon_id,
            bullet->parent_id, E.dynamic && E.R.O ? E.R.O->ID() : 65535,
            mt ? mt->m_Name.c_str() : NULL, bullet->life_time, E.R.element);
        funct(table);
      }
      lua_lock.Leave();
