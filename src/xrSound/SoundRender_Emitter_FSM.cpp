...
void CSoundRender_Emitter::update(float dt) {
  ...
  // demonized: add preplay update, calculate delay based on distance and speed
  // of sound Adaptation for user experience For distances < 30m (configurable
  // via cvar), no delay Then gradually ramp up to delay calculated by math
  // Starting at 90m, full delay kicks in
  // Don't apply delay to already delayed sounds, they are most likely already
  // handled in respective parts of code and scripts
  if (need_preplay_update && m_current_state < stPlaying &&
    ...
    if (_valid(p_source.position) &&
      ...
      static auto CalculateSmoothSoundDelay = [](float distance,
                                                 float speedOfSound,
                                                 float rampStart,
                                                 float rampRange) {
      ...
      auto delay = CalculateSmoothSoundDelay(
          ...
          speedOfSound, soundSmoothingParams::distanceBasedDelayMinDistance,
          60);
      ...
      if (delay > 0.f) {
        ...
        source()->file_name() : "null", magic_enum::enum_name(oldState).data(),
                p_source.position.distance_to(SoundRender->listener_position()),
                oldDelay,
                starting_delay
        );*/
...
BOOL CSoundRender_Emitter::update_culling(float dt) {
  if (b2D) {
    ...
    float occ =
        (owner_data->g_type == SOUND_TYPE_WORLD_AMBIENT)
            ? 1.0f
            : (m_occ_value_ready ? m_current_occ_value
                                 : SoundRender->get_occlusion(p_source.position,
                                                              .2f, occluder));
    m_occ_value_ready = false; // Reset flag
  ...
  // Update smoothing
  smooth_volume =
      .9f * smooth_volume + .1f * (p_source.base_volume * p_source.volume *
                                   (owner_data->s_type == st_Effect
                                        ? psSoundVEffects * psSoundVFactor
                                        : psSoundVMusic * psSoundVMusicFactor) *
