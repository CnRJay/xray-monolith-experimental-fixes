...
public:
  ...
  float m_current_occ_value = 0.f;
  bool m_occ_value_ready = false;
...
public:
  ...
  virtual void set_frequency(float scale) {
    ...
    // demonized: if the sound is short, apply pitch variation, so that stuff
    // like music and most of speech won't be randomized
    ...
    if (fTimeToStop != 0.f)
      fTimeToStop =
          ...
          ((get_length_sec() - (SoundRender->fTimer_Value - fTimeStarted)) /
