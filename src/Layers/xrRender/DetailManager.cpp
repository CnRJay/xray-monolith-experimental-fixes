...
CDetailManager::CDetailManager() {
  ...
  cache		[dm_cache_line][dm_cache_line];	// grid-cache itself Slot
  cache_pool	[dm_cache_size];				// just memory
  for slots */
#endif
}
...
void CDetailManager::UpdateVisibleM() {
  ...
  tbb::enumerable_thread_specific<ThreadVisibles> tls([&]() {
    ThreadVisibles tv;
    for (int i = 0; i < 3; ++i)
      tv.lists[i].resize(objects.size());
    return tv;
  });
