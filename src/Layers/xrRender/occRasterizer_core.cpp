...
void i_order(RasterCtx &Ctx, float *A, float *B, float *C) {
  float *min, *max, *mid;
  if (A[1] <= B[1]) {
    if (B[1] <= C[1]) {
      min = A;
  ...
  Ctx.currentA[0] = min[0] + 2;
  Ctx.currentB[0] = mid[0] + 2;
  Ctx.currentC[0] = max[0] + 2;
  Ctx.currentA[1] = min[1] + 2;
  Ctx.currentB[1] = mid[1] + 2;
  Ctx.currentC[1] = max[1] + 2;
  Ctx.currentA[2] = min[2];
  Ctx.currentB[2] = mid[2];
  Ctx.currentC[2] = max[2];
}
...
// and current color
void i_scan(RasterCtx &Ctx, int curY, float leftX, float lhx, float rightX,
            float rhx, float startZ, float endZ) {
  // calculate span(s)
  ...
  for (; i < limit; i++, Z += dZ) {
    if (shared(Ctx.currentTri, pFrame[i - 1])) {
      // float ZR = (Z+2*pDepth[i-1])*one_div_3;
      if (Z < pDepth[i]) {
        pFrame[i] = Ctx.currentTri;
...
IC void i_section(RasterCtx &Ctx, int Sect, BOOL bMiddle) {
  ...
  if (Sect == BOTTOM) {
    startY = iCeil(Ctx.currentA[1]);
    endY = iFloor(Ctx.currentB[1]) - 1;
    startp1 = startp2 = Ctx.currentA;
    if (bMiddle)
      endY++;

    // check 'endY' for out-of-triangle
    int test = iFloor(Ctx.currentC[1]);
    if (endY >= test)
      endY--;

    // Find the edge differences
    E1[0] = Ctx.currentB[0] - Ctx.currentA[0];
    E2[0] = Ctx.currentC[0] - Ctx.currentA[0];
    E1[1] = Ctx.currentB[1] - Ctx.currentA[1];
    E2[1] = Ctx.currentC[1] - Ctx.currentA[1];
    E1[2] = Ctx.currentB[2] - Ctx.currentA[2];
    E2[2] = Ctx.currentC[2] - Ctx.currentA[2];
  } else {
    startY = iCeil(Ctx.currentB[1]);
    endY = iFloor(Ctx.currentC[1]);
    startp1 = Ctx.currentA;
    startp2 = Ctx.currentB;
    if (bMiddle)
      startY--;

    // check 'startY' for out-of-triangle
    int test = iCeil(Ctx.currentA[1]);
    if (startY < test)
      startY++;

    // Find the edge differences
    E1[0] = Ctx.currentC[0] - Ctx.currentA[0];
    E2[0] = Ctx.currentC[0] - Ctx.currentB[0];
    E1[1] = Ctx.currentC[1] - Ctx.currentA[1];
    E2[1] = Ctx.currentC[1] - Ctx.currentB[1];
    E1[2] = Ctx.currentC[2] - Ctx.currentA[2];
    E2[2] = Ctx.currentC[2] - Ctx.currentB[2];
  }
  ...
  for (; startY <= endY; startY++) {
    i_scan(Ctx, startY, leftX, lhx, rightX, rhx, leftZ, rightZ);
    leftX += left_dX;
    rightX += right_dX;
    leftZ += left_dZ;
    rightZ += right_dZ;
...
u32 occRasterizer::rasterize(occTri *T) {
  ...
  // Rasterize sections
  if (Ctx.currentB[1] - iFloor(Ctx.currentB[1]) > .5f) {
    i_section(Ctx, BOTTOM, 1); // Rasterise First Section
    i_section(Ctx, TOP, 0);    // Rasterise Second Section
  } else {
    i_section(Ctx, BOTTOM, 0); // Rasterise First Section
    i_section(Ctx, TOP, 1);    // Rasterise Second Section
  }
  return Ctx.dwPixels;
}
