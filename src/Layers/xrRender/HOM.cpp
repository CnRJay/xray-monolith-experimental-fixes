...
void CHOM::Load() {
  ...
  for (u32 it = 0; it < CL.getTS(); it++) {
    ...
    if (rT.area < EPS_L) {
      Msg("! Invalid HOM triangle (%f,%f,%f)-(%f,%f,%f)-(%f,%f,%f)", VPUSH(v0),
          VPUSH(v1), VPUSH(v2));
    }
...
BOOL CHOM::visible(vis_data &vis) {
  ...
  if (result) {
    ...
    u32 seed = u32(vis.sphere.P.x) ^ u32(vis.sphere.P.z) ^ frame_current;
    delay = 10 + (seed % 15); // Equivalent to randI(10, 25) i.e. 5*2, 5*5
  } else {
