  ...
  if (g_bootComplete) {
    if (spatialParent) {
      ...
      if (!visibleCheck) {
        ...
        {
                Msg("CKinematics::CalculateBones, object out of frustum, dist
        %.2f, update_rate_k %.2f", dist / dist_k, update_rate_k);
        }*/
      }
      ...
      if (r_optimize_calculate_bones && canBeOptimized() &&
        ...
        /*if (RDEVICE.dwTimeGlobal % 100 < 10)
        {
                Msg("CKinematics::CalculateBones, object canBeOptimized and dist
        > IK_CALC_DIST * dist_k, dist %.2f, update_rate_k %.2f, ssa %.4f", dist
        / dist_k, update_rate_k, ssa);
        }*/
      }
    }
  }
