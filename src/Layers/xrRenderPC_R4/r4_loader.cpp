...
void CRender::level_Unload() {
  ...
  for (u32 i = 0; i < m_fluid_volumes_to_cleanup.size(); ++i) {
    xr_delete(m_fluid_volumes_to_cleanup[i]);
  }
  m_fluid_volumes_to_cleanup.clear();
  ...
  if (FS.exist(fn_game, "$level$", "level.fog_vol")) {
    ...
    if (version == 3) {
      ...
      for (u32 i = 0; i < cnt; ++i) {
        ...
        if (((FHierrarhyVisual *)pRoot)->bDontDelete) {
          m_fluid_volumes_to_cleanup.push_back(pVolume);
        }
      }
    }
