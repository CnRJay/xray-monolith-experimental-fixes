...
{
	// Targets
	if (dwAccumulatorClearMark == Device.frame_data.dwFrame)
	{
		// normal operation - setup
		if (!RImplementation.o.dx10_msaa)
		{
			if (RImplementation.o.fp16_blend) u_setrt(rt_Accumulator, nullptr,NULL, baseZB);
			else u_setrt(rt_Accumulator_temp, nullptr,NULL, baseZB);
		}
		else
		{
			if (RImplementation.o.fp16_blend) u_setrt(rt_Accumulator, nullptr,NULL, rt_MSAADepth->pZRT);
			else u_setrt(rt_Accumulator_temp, nullptr,NULL, rt_MSAADepth->pZRT);
		}
	}
	else
	{
		// initial setup
		dwAccumulatorClearMark = Device.frame_data.dwFrame;

		// clear
		if (!RImplementation.o.dx10_msaa)
			u_setrt(rt_Accumulator, nullptr,NULL, baseZB);
		else
			u_setrt(rt_Accumulator, nullptr,NULL, rt_MSAADepth->pZRT);
...
void CRenderTarget::phase_vol_accumulator()
{
	if (RImplementation.o.ssfx_volumetric)
	{
		...
		u_setrt(rt_Generic_2, nullptr, NULL, nullptr);
	}
	else
	{
		...
		if (!m_bHasActiveVolumetric)
		{
			m_bHasActiveVolumetric = true;
			if (!RImplementation.o.dx10_msaa)
				u_setrt(rt_Generic_2, nullptr, NULL, baseZB);
			else
				u_setrt(rt_Generic_2, nullptr, NULL, RImplementation.Target->rt_MSAADepth->pZRT);
