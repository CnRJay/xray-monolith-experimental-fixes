    ...
    for (u32 o_it = 0; o_it < CObjectSpaceThreadData::r_spatial.size(); o_it++) {
      ...
      if (collidable && (collidable != ignore_object)) {
        ...
        if ((tgt & (rqtObject | rqtObstacle)) && (tp == cftObject) &&
            collidable->collidable.model->_RayQuery(Q, CObjectSpaceThreadData::r_temp))
          return TRUE;
...
BOOL CObjectSpace::RayQuery(collide::rq_results &dest,
  ...
  CObjectSpaceThreadData::r_spatial.clear_not_free();
  return (_res);
}
...
BOOL CObjectSpace::_RayQuery3(collide::rq_results &r_dest,
  ...
  do {
    ...
    if (R.tgt & d_mask) {
      // Traverse object database
      g_SpatialSpace->q_ray(CObjectSpaceThreadData::r_spatial, 0, d_flags, d_rd.start, d_rd.dir,
                            d_rd.range);
      for (u32 o_it = 0; o_it < CObjectSpaceThreadData::r_spatial.size(); o_it++) {
        CObject *collidable = CObjectSpaceThreadData::r_spatial[o_it]->dcast_CObject();
        if (0 == collidable)
      ...
      if (d_rd.range > EPS) {
        ...
        for (u32 o_it = 0; o_it < CObjectSpaceThreadData::r_spatial.size(); o_it++) {
          ...
          if (!((0 == CObjectSpaceThreadData::r_temp.r_count()) ||
                (CObjectSpaceThreadData::r_temp.r_count() && (fis_zero(CObjectSpaceThreadData::r_temp.r_begin()->range, EPS) ||
                                      (CObjectSpaceThreadData::r_temp.r_begin()->range >= 0.f)))))
            Debug.fatal(DEBUG_INFO,
                        "Invalid RayQuery dynamic range: %f (%f). /#2/",
                        CObjectSpaceThreadData::r_temp.r_begin()->range, d_rd.range);
#endif
        }
      }
      if (CObjectSpaceThreadData::r_temp.r_count()) {
...
// RayQueryParallel
//--------------------------------------------------------------------------------
BOOL CObjectSpace::RayQueryParallel(collide::rq_results &r_dest,
                                    const collide::ray_defs &R,
                                    collide::rq_callback *CB, LPVOID user_data,
                                    collide::test_callback *tb,
                                    CObject *ignore_object, xrXRC &xrc_local,
                                    collide::rq_results &r_temp_local,
                                    xr_vector<ISpatial *> &r_spatial_local) {
  // initialize query
  r_dest.r_clear();
  r_temp_local.r_clear();
  ...
  // Test static
  if (R.tgt & s_mask) {
    xrc_local.ray_options(R.flags);
    xrc_local.ray_query(&Static, R.start, R.dir, R.range);
    if (xrc_local.r_count()) {
      CDB::RESULT *_I = xrc_local.r_begin();
      CDB::RESULT *_E = xrc_local.r_end();
      for (; _I != _E; _I++)
        r_temp_local.append_result(rq_result().set(0, _I->range, _I->id));
    }
  }
  // Test dynamic
  if (R.tgt & d_mask) {
    // Traverse object database
    g_SpatialSpace->q_ray(r_spatial_local, 0, d_flags, R.start, R.dir, R.range);
    for (u32 o_it = 0; o_it < r_spatial_local.size(); o_it++) {
      CObject *collidable = r_spatial_local[o_it]->dcast_CObject();
      if (0 == collidable)
        continue;
      if (collidable == ignore_object)
        continue;
      ...
      if (((R.tgt & (rqtObject | rqtObstacle)) && (tp == cftObject)) ||
        ...
        if (tb && !tb(R, collidable, user_data))
          continue;
        cform->_RayQuery(R, r_temp_local);
      }
    }
  }
  if (r_temp_local.r_count()) {
    r_temp_local.r_sort();
    collide::rq_result *_I = r_temp_local.r_begin();
    collide::rq_result *_E = r_temp_local.r_end();
    for (; _I != _E; _I++) {
      r_dest.append_result(*_I);
      if (!(CB ? CB(*_I, user_data) : TRUE))
        return r_dest.r_count();
